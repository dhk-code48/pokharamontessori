...
export const AuthorForm: React.FC<AuthorFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (data: AuthorFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/${params.siteId}/authors/${params.authorId}`, data);
      } else {
        await axios.post(`/api/${params.siteId}/authors`, data);
      }
      router.refresh();
      router.push(`/${params.siteId}/authors`);
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.siteId}/authors/${params.authorId}`);
      router.refresh();
      router.push(`/${params.siteId}/authors`);
